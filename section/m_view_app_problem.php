      ...
      <table id="problem_table" class="table table-bordered table-striped table-responsive">
        ...
        <tbody>
          ...
          <tr data-appid="<?php echo addslashes($app_id);?>">
			...
			<td>
            <?php
                $resultjalloted=mysqli_query($conn,"SELECT zero_score FROM manager WHERE app_id='$app_id' and manager.manager_id='$username'");
                while($rowjalloted = $resultjalloted->fetch_assoc())
                {
                ?>
                    <span class="blue-box">
                        <?php echo $rowjalloted['zero_score'];?>
                    </span>
                <?php
				}
			    ?>
            ...
            <td>
              <button type="submit" class="btn btn-danger z-optionbtn" data-i="<?php echo $count;?>" style="float:left;">Open
